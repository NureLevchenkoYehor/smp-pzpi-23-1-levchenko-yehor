...
CREATE TABLE IF NOT EXISTS [UserIdentities] (
    ...
    [user_identity_password_hash] TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS [CustomerProfiles] (
    ...
    [customer_profile_image_id] INTEGER,
    [customer_identity_id] INTEGER NOT NULL,
    FOREIGN KEY ([customer_profile_image_id]) REFERENCES Images([image_id]) ON DELETE SET NULL,
    FOREIGN KEY ([customer_identity_id]) REFERENCES UserIdentities([user_identity_id]) ON DELETE CASCADE,
    UNIQUE([customer_identity_id])
);

CREATE INDEX IF NOT EXISTS [idx_customer_identity_id] ON [CustomerProfiles]([customer_identity_id]);
